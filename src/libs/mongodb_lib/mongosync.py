...
class MongoSync:
    ...
    def __init__(
        ...
        self.mongosync_port = (
            mongosync_port
            if mongosync_port
            else self.staged_source.parameters.mongosync_port
        )
        ...
        self.mongodb_obj = MongoDB(
            repository=self.repository, resource=self.resource
        )
    ...
    def check_staging_parameters(self) -> None:
        ...
        if not self.os_lib_obj.check_file_dir_exists(
            path=self.repository.mongosync_path, is_directory=False
        ):
            error_msg = (
                f"Mongosync path: "
        ...
        mongosync_version = (
            self.os_lib_obj.get_binary_version(
        ...
        if not helpers.compare_version(
            ...
            error_msg = (
                f"Mongosync version: {mongosync_version} not "
                f"supported. Only "
        ...
        if not helpers.compare_version(
            ...
            error_msg = (
                f"MongoDB version: {mongod_version} not supported "
        ...
        if MongoSyncConstants.mongodb_supported_type not in mongod_type:
            error_msg = (
                "Selected MongoDB repository does not seem to be "
        ...
        if (
            self.staged_source.parameters.storage_engine
            != MongoSyncConstants.supported_engine
        ):
            error_msg = (
                f"Storage Engine : "
        ...
        if not self.os_lib_obj.check_port_available(self.mongosync_port):
            error_msg = (
                f"Port number: "
    ...
    def check_source_parameters(self) -> None:
        ...
        if not helpers.compare_version(
            mongodb_version, self.repository.version[1:], "="
        ):
            error_msg = (
                f"Source database version {mongodb_version} does "
        ...
        # mongodb should be enterprise edition
        if (
            MongoSyncConstants.mongodb_supported_type
            not in self.mongodb_obj.get_type(
                host_conn_string=self.staged_source.parameters.src_mongo_host_conn,
                username=self.staged_source.parameters.src_db_user,
                password=self.staged_source.parameters.src_db_password,
            )
            ...
            error_msg = (
                " Source database provided is not Enterprise."
    ...
    def get_mongosync_data_dir(self) -> str:
        ...
        :rtype: ``str``
        """
        mongosync_dir = os.path.join(
            self.staged_source.parameters.mount_path, ".delphix", "mongosync"
        )
    ...
    def get_mongosync_conf_path(self) -> str:
        ...
        conf_path = os.path.join(
            self.get_mongosync_data_dir(), "mongosync.conf"
        )
        if self.mongosync_id:
            conf_path = conf_path.replace(
                ".conf", f"_{self.mongosync_id}.conf"
            )
        return conf_path
    ...
    def create_mongosync_conf(self) -> None:
        ...
        dst_conn_string = MongoDB.get_standard_conn_string(
            ...
            if self.staged_source.parameters.enable_ssl_tls
            else None,
        )
    ...
    def start_mongosync(self, create_conf: bool = True) -> None:
        ...
        start_cmd = MongoSyncConstants.mongosync_start_cmd.format(
            mongosync_path=self.repository.mongosync_path, conf_path=conf_path
        )
        ...
        if res.exit_code != 0:
            mongosync_error = helpers.get_error_from_logfile(
                connection=self.staged_source.staged_connection,
                env={},
                command=start_cmd,
                conf_path=conf_path,
            ...
            raise plugin_error.PluginError(
                f"""
                Mongosync start failed \n
                Error: {res.stderr} \n
                Mongosync Log Error : {mongosync_error}
                """
            )
    ...
    def start_sync(self, wait_cancommit: bool = False) -> bool:
        ...
        if (
            int(http_code) == 200
            and response_json["progress"]["state"] != "RUNNING"
        ):
    ...
    def pause_sync(
        self,
    ) -> Tuple[int, str, Dict[str, Union[List, str, int, dict]]]:
        """
        Pauses sync.
        ...
        except plugin_error.CurlException as exc:
            shell_error, mongosync_error = self._handle_error(
                exit_code=exc.exit_code, exc_message=exc.message
            )
            raise plugin_error.PluginError(
                f"""
                Mongosync pause API failed \n
                Error : {shell_error} \n
                Mongosync log error: {mongosync_error}
                """
            )
    ...
    def _handle_error(
        self, exit_code: int, exc_message: str
    ) -> Tuple[str, str]:
        """
        Creates meaningful error messages from shell and mongosync logs.

        :param exit_code: Exit code from shell execution.
        :type int
        :param exc_message: Stderr message from shell execution.
        :type str

        :return: Tuple containing shell_error, mongosync_error
        :rtype: ``Tuple[str, str]``
        """
        ...
        shell_error = ""
        if exit_code == MongoSyncConstants.curl_connection_refused_code:
            shell_error = """
            Mongosync API service is not responding.\n
            Either API service is down or Mongosync process is no more running.
            """
        else:
            try:
                shell_error = exc_message.split("\n")[-2]
            except IndexError:
                shell_error = exc_message.strip()

        return shell_error, mongosync_error
