...
def extract_replicaset_backup(backupfile, mount_path, connection):
    cmd = "tar -{}xf {} -C {}/s0m0 --strip-components 1".format(
        "z" if backupfile.endswith(".tar.gz") else "", backupfile, mount_path
    )
    res = execute_bash_cmd_silent(connection, cmd, {})


def gen_shard_config_list(
    nodes,
    start_portpool,
    cmax,
    smax,
...
def _handle_exit_code(
    ...
    if exit_code == 0:
        if silent_status == "N":
            logger.debug(
                "Code : {}, Response : {}".format(exit_code, std_output)
            )
        ...
        if callback_func:
            logger.debug(
                "Executing call back. Seems some exception is observed. Validating last error..."
            )
            try:
                ...
                logger.debug(
                    "Failed to execute call back function with error: {}".format(
                        err.message
                    )
                )
    ...
    logger.debug(
        "cmd Exit Code: {}, Error Msg : {}, Output Msg : {}".format(
            exit_code, std_err, std_output
        )
    )
    add_debug_space()
...
def execute_bash_cmd_silent_status(connection, cmd, env):
    ...
    exit_code = res.exit_code
    logger.debug(
        "Value for result: cmd: {} ---- outputmsg: {} ---- errmsg: {} ---- exit_code: {} ".format(
            cmd, outputmsg, errmsg, exit_code
        )
    )
...
def gen_shardserver_setting_list(
    shard_config_list,
    sourceobj,
    shard_count,
    mount_path,
    ...
    for shardnum in range(shard_count):
        ...
        shardservercfg = "{}/{}:{}".format(
            snm0_replica_name, snm0_host_name.replace("\n", ""), snm0_port
        )
...
def gen_configsvrstring(dataset_type, sourceobj, shard_config_list):
    ...
    configsvrstring = "{}/{}:{}".format(
        c0m0_replica_name, c0m0_host_name, c0m0_port
    )
    ...
    if replicaset:
        ...
        configsvrstring = (
            configsvrstring + "," + "{}:{}".format(c0m1_host_name, c0m1_port)
        )
        ...
        configsvrstring = (
            configsvrstring + "," + "{}:{}".format(c0m2_host_name, c0m2_port)
        )
...
def gen_mongo_conf_files(dataset_type, sourceobj, shard_config_list, snapshot):
    ...
    for rec in json_result.split("},"):
        ...
        if mongo_process_type == "mongos":
            # mongo_cmd = "mongos"
            mongo_cmd = "{}/mongos".format(
                os.path.dirname(sourceobj.mongo_install_path)
            )
        else:
        ...
        logger.info(
            "After mongo_process_type - mongo_cmd = {}".format(mongo_cmd)
        )

        dbpath = "{}/{}".format(mount_path, mongod_dirname)
        cfgdir = "{}/cfg".format(mount_path)
        logdir = "{}/logs".format(mount_path)
        cfgfile = "{}/dlpx.{}.{}.conf".format(
            cfgdir, mongod_dirname, mongod_port
        )
        logfile = "{}/dlpx.{}.{}.mongod.log".format(
            logdir, mongod_dirname, mongod_port
        )
        ...
        mongo_cmd = add_net(
            mongo_cmd, bind_ip, mongod_port, enable_ssl_tls, ssl_tls_params
        )
        ...
        logger.info(
            "After add_keyfile_auth - mongo_cmd = {}".format(mongo_cmd)
        )
        add_debug_space()

        mongo_cmd = add_cluster_auth(
            mongo_cmd, cluster_auth_mode, keyfile_path
        )
        logger.info(
            "After add_cluster_auth - mongo_cmd = {}".format(mongo_cmd)
        )
        add_debug_space()
        ...
        mongo_cmd = add_set_parameters(
            mongo_cmd, enable_setparams, setparam_params
        )
        logger.info(
            "After add_set_parameters - mongo_cmd = {}".format(mongo_cmd)
        )
        add_debug_space()
        ...
        if mongo_process_type == "mongos":
            ...
            if source_encrypted:
                ...
                if encryption_method == "KMIP":
                    ...
                    logger.info(
                        "After add_encryption_kmip - mongo_cmd = {}".format(
                            mongo_cmd
                        )
                    )

                    if d_source_type not in [
                        "offlinemongodump",
                        "onlinemongodump",
                        "extendedcluster",
                    ]:
                        cmd = "cat {}/{}/restoreInfo.txt|grep 'Master Key UUID'".format(
                            mount_path, mongod_dirname
                        )
                        ...
                        mongo_cmd = (
                            mongo_cmd
                            + " --kmipKeyIdentifier {}".format(kmip_key_id)
                        )
                        logger.info(
                            "After add_encryption_kmip kmip_key_id - mongo_cmd = {}".format(
                                mongo_cmd
                            )
                        )

                elif encryption_method == "KeyFile":
                    mongo_cmd = add_encryption_keyfile(
                        mongo_cmd, encryption_keyfile
                    )
                    logger.info(
                        "After add_encryption_keyfile - mongo_cmd = {}".format(
                            mongo_cmd
                        )
                    )
            add_debug_space()
        ...
        if mongo_process_type == "shard" or mongo_process_type == "repset":
            cmd = "cat {}/s{}m0/restoreInfo.txt|grep 'Replica Set:'".format(
                mount_path, mongod_dirname[1:2]
            )
            res = execute_bash_cmd(mongod_conn, cmd, {})
            repset_name = res.split(":")[1].strip()
        elif mongo_process_type == "config":
            cmd = "cat {}/c{}m0/restoreInfo.txt|grep 'Replica Set:'".format(
                mount_path, mongod_dirname[1:2]
            )
            res = execute_bash_cmd(mongod_conn, cmd, {})
            repset_name = res.split(":")[1].strip()
        ...
        if mongo_process_type == "shard":
            mongo_cmd = "{} --replSet {} --shardsvr".format(
                mongo_cmd, repset_name
            )
        elif mongo_process_type == "config":
            mongo_cmd = "{} --replSet {} --configsvr".format(
                mongo_cmd, repset_name
            )
        ...
        if mongo_process_type == "mongos":
            ...
            cfgdblist = gen_configsvrstring(
                dataset_type, sourceobj, shard_config_list
            )
            mongo_cmd = "{} --configdb {}".format(mongo_cmd, cfgdblist)
            logger.info(
                "After configsvrstring - mongo_cmd = {}".format(mongo_cmd)
            )
        ...
        mongo_cmd = "{} --outputConfig |grep -v outputConfig > {}".format(
            mongo_cmd, cfgfile
        )
        ...
        fix_mongod_cmd = "sed -i 's/replSet: {}/replSetName: {}/g' {}".format(
            repset_name, repset_name, cfgfile
        )
...
def start_sharded_mongo(dataset_type, sourceobj):
    ...
    if dataset_type == "Virtual":
        cfgfile = "{}/.delphix/.tgt_vdbcfg.txt".format(mount_path)
        connection = sourceobj.connection
        cmd = "cat {}/.delphix/.tgt_config.txt|grep DSOURCE_TYPE".format(
            mount_path
        )
        res = execute_bash_cmd(connection, cmd, {})
    ...
    for rec in json_result.split("},"):
        ...
        if jsonobj["type"] == "mongos":
            ...
            cmd = "{}/mongos -f {}".format(
                os.path.dirname(sourceobj.mongo_install_path), start_string
            )
        else:
    ...
    if d_source_type == "shardedsource":
        ...
        if mongos_port != start_portpool:
            ...
            cmd = "{} --port {} --eval 'sh.startBalancer()'".format(
                mongo_shell_cmd, mongos_port
            )
...
def stop_sharded_mongo(dataset_type, sourceobj):
    ...
    if d_source_type == "shardedsource":
        try:
            # Stop Balancer
            cmd = "{} --port {} --eval 'sh.stopBalancer()'".format(
                mongo_shell_cmd, mongos_port
            )
    ...
    if d_source_type == "stagingpush" and dataset_type == "Staging":
        ...
        for rec in json_result.split("},"):
            ...
            mongod_conn = get_node_conn(
                sourceobj, jsonobj["node"], dataset_type
            )
            mongod_dirname = jsonobj["dirname"]
...
def get_status_sharded_mongo(dataset_type, sourceobj):
    ...
    if dataset_type == "Virtual":
        ...
        cmd = "cat {}/.delphix/.stg_config.txt | grep DSOURCE_TYPE".format(
            mount_path
        )
    ...
    try:
        ...
        for rec in json_result.split("},"):
            ...
            if int(res) == 0:
                logger.debug(
                    "Process for {}/cfg/dlpx.{}.{}.conf not running".format(
                        mount_path, mongod_dirname, mongod_port
                    )
                )
                active_status_flag = 1
        ...
        logger.debug(
            "ERROR : Checking status of sharded mongo - Mongos {} at {}".format(
                sourceobj.parameters.mongos_port, mount_path
            )
        )
        logger.debug(e)
        active_status_flag = 1
...
def gen_mongo_cmd(dataset_type, sourceobj, hostname, use_source=False):
    ...
    if use_source:
        ...
        if dataset_type == "Staging":
            ...
            cmd = "cat {}/.delphix/.stg_config.txt|grep 'MONGO_DB_USER:'|cut -d':' -f2".format(
                mount_path
            )
    ...
    logger.debug(
        "enable_ssl_tls: {}".format(sourceobj.parameters.enable_ssl_tls)
    )
    if sourceobj.parameters.enable_ssl_tls:
        mongo_shell_cmd = "{} --tls --host {} --tlsCertificateKeyFile {} --tlsCAFile {}".format(
            mongo_shell_cmd, hostname, client_tls_cert, client_tls_cacert
        )

    if sourceobj.parameters.keyfile_path:
        mongo_shell_cmd = "{} --username {} --password {} --authenticationDatabase admin".format(
            mongo_shell_cmd, mongo_db_user, mongo_db_password
        )
...
def fsync_lock_sharded_mongo(sourceobj, dataset_type="Virtual"):
    ...
    for rec in json_result.split("},"):
        ...
        if jsonobj["type"] != "mongos":
            cmd = "{} --port {} --eval 'db.fsyncLock()'".format(
                mongo_shell_cmd, mongod_port
            )
...
def fsync_unlock_sharded_mongo(sourceobj, dataset_type="Virtual"):
    ...
    for rec in json_result.split("},"):
        ...
        if jsonobj["type"] != "mongos":
            cmd = "{} --port {} --eval 'db.fsyncUnlock()'".format(
                mongo_shell_cmd, mongod_port
            )
...
def setup_mongos(
    ...
    logfile = "{}/dlpx.mgs{}.{}.mongod.log".format(
        logdir, membernum, mongos_port
    )
    ...
    cmd = "{} --port {} --quiet --eval 'sh.status()'".format(
        sourceobj.mongo_shell_path, mgs0_port
    )
    res = execute_bash_cmd(rx_connection, cmd, {})

    # cmd = "mongo --port {} --quiet --eval 'sh.startBalancer()'".format(mgs0_port)
    cmd = "{} --port {} --quiet --eval 'sh.startBalancer()'".format(
        sourceobj.mongo_shell_path, mgs0_port
    )
    res = execute_bash_cmd(rx_connection, cmd, {})

    # cmd = "mongo admin --port {} --quiet --eval 'db.runCommand(\"getShardMap\")'".format(c0m0_port)
    cmd = "{} admin --port {} --quiet --eval 'db.runCommand(\"getShardMap\")'".format(
        sourceobj.mongo_shell_path, c0m0_port
    )
    res = execute_bash_cmd(rx_connection, cmd, {})


def setup_config_member(
    sourceobj,
    rx_connection,
    mount_path,
    confignum,
    ...
    cfgdir = "{}/cfg".format(mount_path)
    logdir = "{}/logs".format(mount_path)
    cfgfile = "{}/dlpx.c{}m{}.{}.conf".format(
        cfgdir, confignum, membernum, cfg_port
    )
    logfile = "{}/dlpx.c{}m{}.{}.mongod.log".format(
        logdir, confignum, membernum, cfg_port
    )
    ...
    logger.debug(
        "{},{},{},{},{}".format(
            dbpath, cfg_port, shardcount, cfgfile, hostname
        )
    )

    cmd = "cat {}/c{}m{}/restoreInfo.txt|grep 'Replica Set:'".format(
        mount_path, confignum, membernum
    )
    res = execute_bash_cmd(rx_connection, cmd, {})
    ...
    if encryption_method is None:
        cmd = "{} --bind_ip 0.0.0.0 --port {} --dbpath {} --logpath {} --fork".format(
            sourceobj.mongo_install_path, cfg_port, dbpath, logfile
        )
    ...
    if (
        ...
        for i in range(shardcount):
            ...
            logger.debug(
                "{},{},{}".format(shardmember, snm0_port, shard_dbpath)
            )
            logger.debug("adjust_config_shardinfo - Start")
            adjust_config_shardinfo(
                sourceobj,
                rx_connection,
                shard_dbpath,
                cfg_port,
...
def setup_config_replset_members(
    shard_config_list,
    sourceobj,
    mount_path,
    encryption_method,
    ...
    for i in range(1, 3):
        ...
        logdir = "{}/logs".format(mount_path)
        cfgfile = "{}/dlpx.c{}m{}.{}.conf".format(
            cfgdir, confignum, membernum, cnmn_port
        )
        logfile = "{}/dlpx.c{}m{}.{}.mongod.log".format(
            logdir, confignum, membernum, cnmn_port
        )

        logger.debug(
            "{},{},{},{}".format(dbpath, cnmn_port, cfgfile, cnmn_host_name)
        )

        cmd = "cp -p {}/c0m0/restoreInfo.txt {}/c{}m{}/restoreInfo.txt".format(
            mount_path, mount_path, confignum, membernum
        )
        ...
        if encryption_method is None:
            cmd = "{} --bind_ip 0.0.0.0 --port {} --dbpath {} --logpath {} --fork".format(
                sourceobj.mongo_install_path, cnmn_port, dbpath, logfile
            )
            restart_mongod_cmd = "{} --bind_ip 0.0.0.0 --port {} --dbpath {} --logpath {} --fork --replSet {} --configsvr".format(
                sourceobj.mongo_install_path,
                cnmn_port,
        ...
        logger.info(
            "Add replicaset Member c{}m{} to replicaset {}".format(
                confignum, membernum, config_repset_name
            )
        )
        cmd = "{} --port {} --eval 'rs.add(\"{}:{}\")'".format(
            sourceobj.mongo_shell_path, c0m0_port, cnmn_host_name, cnmn_port
        )
        cmd_response = execute_bash_cmd(c0m0_conn, cmd, {})
        logger.info(
            "Add replicaset Member c{}m{} to replicaset {} - done".format(
                confignum, membernum, config_repset_name
            )
        )
...
def setup_shard_replset_members(
    shard_config_list,
    virtual_source,
    mount_path,
    encryption_method,
    base_enc_params_list_string,
    shard_count,
    dataset_type,
):
    ...
    for shardnum in range(shard_count):
        ...
        if encryption_method == "KMIP":
            kmip_key_id = get_kmip_key_id(
                "{}/s{}m0".format(mount_path, shardnum), snm0_conn
            )
        ...
        for i in range(1, 3):
            ...
            add_debug_heading_block(
                "Member: s{}m{}".format(shardnum, membernum)
            )
            snmn_port = get_shard_port(
            ...
            cfgdir = "{}/cfg".format(mount_path)
            logdir = "{}/logs".format(mount_path)
            cfgfile = "{}/dlpx.s{}m{}.{}.conf".format(
                cfgdir, shardnum, membernum, snmn_port
            )
            logfile = "{}/dlpx.s{}m{}.{}.mongod.log".format(
                logdir, shardnum, membernum, snmn_port
            )

            logger.debug("{},{},{}".format(dbpath, cfgfile, snmn_host_name))

            # Create restoreInfo.txt file and add correct encryption keyid for KMIP
            cmd = "cp -p {}/s{}m0/restoreInfo.txt {}/s{}m{}/restoreInfo.txt".format(
                mount_path, shardnum, mount_path, shardnum, membernum
            )
            res = execute_bash_cmd(snmn_conn, cmd, {})
            cmd = (
                "sed -i '/Master Key UUID/d' {}/s{}m{}/restoreInfo.txt".format(
                    mount_path, shardnum, membernum
                )
            )
            res = execute_bash_cmd(snmn_conn, cmd, {})
            if encryption_method:
                cmd = "echo 'Master Key UUID: {}' >> {}/s{}m{}/restoreInfo.txt".format(
                    kmip_key_id, mount_path, shardnum, membernum
                )
                res = execute_bash_cmd(snmn_conn, cmd, {})
            logger.info("Copied and Adjusted restoreInfo.txt")

            cmd = "cat {}/s{}m{}/restoreInfo.txt|grep 'Replica Set:'".format(
                mount_path, shardnum, "0"
            )
            res = execute_bash_cmd(snm0_conn, cmd, {})
            ...
            shutdown_server(
                snmn_conn, virtual_source.mongo_shell_path, snmn_port
            )
            ...
            # Add Member to replicaset
            logger.info(
                "Add replicaset Member s{}m{} to replicaset {}".format(
                    shardnum, membernum, shard_repset_name
                )
            )
            cmd = "{} --port {} --eval 'rs.add(\"{}:{}\")'".format(
                virtual_source.mongo_shell_path,
                snm0_port,
                snmn_host_name,
                snmn_port,
            )
            ...
            logger.info(
                "Add replicaset Member s{}m{} to replicaset {} - done".format(
                    shardnum, membernum, shard_repset_name
                )
            )
...
def add_replset_members(
    ...
    for i in range(1, 3):
        ...
        logfile = "{}/dlpx.s0m{}.{}.mongod.log".format(
            logdir, membernum, snmn_port
        )

        logger.debug(
            "{},{},{},{}".format(dbpath, snmn_port, cfgfile, snmn_host_name)
        )

        cmd = "cp -p {}/s0m0/restoreInfo.txt {}/s0m{}/restoreInfo.txt".format(
            mount_path, mount_path, membernum
        )
        res = execute_bash_cmd(s0m0_conn, cmd, {})

        cmd = "cat {}/s0m0/restoreInfo.txt|grep 'Replica Set:'".format(
            mount_path
        )
        res = execute_bash_cmd(s0m0_conn, cmd, {})
        ...
        # Add Member to replicaset
        logger.info(
            "Add replicaset Member s0m{} to replicaset {}".format(
                membernum, repset_name
            )
        )
        cmd = "{} --port {} --eval 'rs.add(\"{}:{}\")'".format(
            sourceobj.mongo_shell_path, s0m0_port, snmn_host_name, snmn_port
        )
        cmd_response = execute_bash_cmd(s0m0_conn, cmd, {})
        logger.info(
            "Add replicaset Member s0m{} to replicaset {} - done".format(
                membernum, repset_name
            )
        )
...
def adjust_config_shardinfo(
    ...
    logger.debug(
        "shardserver_setting_list:{}".format(shardserver_setting_list)
    )
    ...
    cmd = '{} config --port {} --quiet --eval \'db.shards.updateOne({{ _id : "{}" }},{{ $set : {{ host : "{}", state : 1 }} }})\''.format(
        sourceobj.mongo_shell_path, cfg_port, shard_repset_name, shard_host_cfg
    )
    res = execute_bash_cmd(rx_connection, cmd, {})


def setup_shard_member(
    sourceobj,
    rx_connection,
    mount_path,
    shardnum,
    ...
    logdir = "{}/logs".format(mount_path)
    cfgfile = "{}/dlpx.s{}m{}.{}.conf".format(
        cfgdir, shardnum, membernum, snm0_port
    )
    logfile = "{}/dlpx.s{}m{}.{}.mongod.log".format(
        logdir, shardnum, membernum, snm0_port
    )
    ...
    logger.debug(
        "{},{},{},{},{}".format(
            dbpath, snm0_port, shardcount, cfgfile, hostname
        )
    )
    ...
    if encryption_method is None:
        cmd = "{} --bind_ip 0.0.0.0 --port {} --dbpath {} --logpath {} --fork".format(
            sourceobj.mongo_install_path, snm0_port, dbpath, logfile
        )
        restart_mongod_cmd = "{} --bind_ip 0.0.0.0 --port {} --dbpath {} --logpath {} --fork --replSet {} --shardsvr".format(
            sourceobj.mongo_install_path,
            snm0_port,
    ...
    cmd = "{} local --port {} --quiet --eval 'db.dropDatabase()'".format(
        sourceobj.mongo_shell_path, snm0_port
    )
    res = execute_bash_cmd(rx_connection, cmd, {})
    ...
    cmd = '{} admin --port {} --quiet --eval \'db.system.version.updateOne({{ "_id" : "shardIdentity" }},{{ $set :{{ "configsvrConnectionString" : "{}" }} }})\''.format(
        sourceobj.mongo_shell_path, snm0_port, configsvrstring
    )
    res = execute_bash_cmd(rx_connection, cmd, {})
    ...
    # Create YAML file
    cmd = "{} --outputConfig |grep -v outputConfig > {}".format(
        restart_mongod_cmd, cfgfile
    )
    res = execute_bash_cmd(rx_connection, cmd, {})

    # This may be bug so added to fix proactively
    fix_mongod_cmd = "sed -i 's/replSet: {}/replSetName: {}/g' {}".format(
        shard_repset_name, shard_repset_name, cfgfile
    )
    logger.debug("fix_mongod_cmd: {}".format(fix_mongod_cmd))
...
def setup_replicaset_member(
    ...
    if dsource_type == "extendedcluster":
        ...
        cmd = "{} --dbpath {} --port {} --shutdown".format(
            sourceobj.mongo_install_path, dbpath, cfg_port
        )
        res = execute_bash_cmd(rx_connection, cmd, {})
        time.sleep(3)
        ...
        try:
            cmd = "cat {}/s0m0/restoreInfo.txt|grep 'Last Oplog Applied:'".format(
                mount_path
            )
    ...
    if enable_ssl_tls:
        ...
        for ssl_tls_params_rec in ssl_tls_params:
            if ssl_tls_params_rec["value"].upper() == "TRUE":
                ssl_tls_params_list_string = (
                    ssl_tls_params_list_string
                    + " --{}".format(ssl_tls_params_rec["property_name"])
                )
            else:
                ssl_tls_params_list_string = (
                    ssl_tls_params_list_string
...
def add_cluster_auth(mongo_cmd, cluster_auth_mode, keyfile_path):
    ...
    if cluster_auth_mode == "None":
        ...
        mongo_cmd = "{} --clusterAuthMode {}".format(
            mongo_cmd, cluster_auth_mode
        )
    return mongo_cmd
    ...
    if enable_ldap:
        ...
        for ldap_params_rec in ldap_params:
            if ldap_params_rec["value"].upper() == "TRUE":
                ldap_params_list_string = (
                    ldap_params_list_string
                    + " --{}".format(ldap_params_rec["property_name"])
                )
            else:
                if (
    ...
    if enable_setparams:
        ...
        for set_params_rec in setparam_params:
            if set_params_rec["value"].upper() == "TRUE":
                set_params_list_string = (
                    set_params_list_string
                    + " --{}".format(set_params_rec["property_name"])
                )
            else:
                set_params_list_string = (
                    set_params_list_string
    ...
    if kmip_params:
        ...
        for kmip_params_rec in kmip_params:
            if kmip_params_rec["value"].upper() == "TRUE":
                kmip_params_list_string = (
                    kmip_params_list_string
                    + " --{}".format(kmip_params_rec["property_name"])
                )
            else:
                kmip_params_list_string = (
                    kmip_params_list_string
                    + " --{} {}".format(
                        kmip_params_rec["property_name"],
...
def add_encryption_keyfile(mongo_cmd, enc_key_file_name):
    if enc_key_file_name:
        mongo_cmd = "{} --enableEncryption".format(mongo_cmd)
        mongo_cmd = "{} --encryptionKeyFile {}".format(
            mongo_cmd, enc_key_file_name
        )
    return mongo_cmd
    ...
    if enable_auditlog:
        ...
        for auditlog_params_rec in auditlog_params:
            if auditlog_params_rec["value"].upper() == "TRUE":
                auditlog_params_list_string = (
                    auditlog_params_list_string
                    + " --{}".format(auditlog_params_rec["property_name"])
                )
            else:
                auditlog_params_list_string = (
                    auditlog_params_list_string
...
def gen_config_files(
    dataset_type, sourceobj, shard_config_list, snapshot=None
):
    if dataset_type == "Virtual":
        connection = sourceobj.connection
        cmd = "cat {}/.delphix/.tgt_config.txt|grep DSOURCE_TYPE".format(
            sourceobj.parameters.mount_path
        )
        res = execute_bash_cmd(connection, cmd, {})
        d_source_type = res.split(":")[1].strip()
    elif dataset_type == "Staging":
        connection = sourceobj.staged_connection
        cfgfile = "{}/.delphix/.stg_dsourcecfg.txt".format(
            sourceobj.parameters.mount_path
        )
        d_source_type = sourceobj.parameters.d_source_type
    ...
    if dataset_type == "Virtual":
        gen_mongo_conf_files(
            dataset_type, sourceobj, shard_config_list, snapshot
        )
    else:
    ...
    if d_source_type == "stagingpush" and dataset_type == "Staging":
        logger.info(
            "++++++++++ Not Applicable for Staging-Stagingpush ++++++++++"
        )
    else:
...
def setup_dataset(sourceobj, dataset_type, snapshot, dsource_type):
    ...
    if dsource_type == "shardedsource":
        ...
        dsource_type == "nonshardedsource"
        or dsource_type == "offlinemongodump"
        or dsource_type == "onlinemongodump"
        or dsource_type == "extendedcluster"
        or dsource_type == "stagingpush"
    ):
    ...
    if dsource_type == "shardedsource":
        if dataset_type == "Staging":
            cmd = 'echo "{}" > {}/.delphix/.stg_dsourcecfg.txt'.format(
                shard_config_list, mount_path
            )
        elif dataset_type == "Virtual":
            cmd = 'echo "{}" > {}/.delphix/.tgt_vdbcfg.txt'.format(
                shard_config_list, mount_path
            )
        res = execute_bash_cmd(rx_connection, cmd, {})
    elif dsource_type in [
        ...
        if dataset_type == "Staging":
            cmd = 'echo "{}" > {}/.delphix/.stg_dsourcecfg.txt'.format(
                replicaset_config_list, mount_path
            )
        elif dataset_type == "Virtual":
            cmd = 'echo "{}" > {}/.delphix/.tgt_vdbcfg.txt'.format(
                replicaset_config_list, mount_path
        ...
        if dsource_type == "shardedsource":
            if sourceobj.parameters.enable_clustersync:
                ...
                _generate_restoreinfo_txt_for_all_shards(
                    sourceobj.os_lib_obj, mount_path, source_shard_details_list
                )
                logger.debug(
                    "Successfully generated restoreInfo.txt for all " "shards."
                )
                ...
                _add_shard_id_in_shard_config_list(
                    sourceobj.os_lib_obj, shard_config_list
                )
                logger.debug(
                    "Successfully added shard ids to " "shard_config_list."
                )
                logger.debug(shard_config_list)
            else:
                # restore backups
                extract_shard_config_backup(
                    config_backupfile, mount_path, 0, rx_connection
                )
                shardnum = 0
                for shardfile in sourceobj.parameters.shard_backupfiles:
                    extract_shard_backup(
                        shardfile["backupfile"],
                        mount_path,
                        shardnum,
                        rx_connection,
                    )
                ...
                shardnum = shardnum - 1
                logger.debug(
                    "Extracted Configserver / Shard backups successfully"
                )
        elif dsource_type == "nonshardedsource":
            # restore backups
            extract_replicaset_backup(
                config_backupfile, mount_path, rx_connection
            )
    ...
    if dsource_type == "shardedsource":
        ...
        c0m0_replica_name = execute_bash_cmd(c0m0_conn, cmd, {})
        c0m0_replica_name = "{}".format(
            c0m0_replica_name.split(":")[1].strip()
        )
        logger.debug("c0m0_replica_name={}".format(c0m0_replica_name))
        configsvrstring = "{}/{}:{}".format(
            c0m0_replica_name, c0m0_host_name, c0m0_port
        ...
        for shardserver_setting in shardserver_setting_list:
            logger.debug(
                "shardserver_setting:  {}".format(shardserver_setting)
            )
        add_debug_space()

    if dataset_type == "Staging":
        if dsource_type == "shardedsource":
            cmd = 'echo "SHARD_COUNT:{}" > {}/.delphix/{}'.format(
                smax, mount_path, dataset_cfgfile
            )
            res = execute_bash_cmd(rx_connection, cmd, {})
        elif dsource_type == "nonshardedsource":
            cmd = "cat /dev/null > {}/.delphix/{}".format(
                mount_path, dataset_cfgfile
            )
            res = execute_bash_cmd(rx_connection, cmd, {})

        cmd = 'echo "DSOURCE_TYPE:{}" >> {}/.delphix/{}'.format(
            sourceobj.parameters.d_source_type, mount_path, dataset_cfgfile
        )
        res = execute_bash_cmd(rx_connection, cmd, {})

        cmd = 'echo "MONGO_DB_USER:{}" >> {}/.delphix/{}'.format(
            sourceobj.parameters.mongo_db_user, mount_path, dataset_cfgfile
        )
        res = execute_bash_cmd(rx_connection, cmd, {})
        ...
        if dsource_type == "shardedsource":
            ...
            res = execute_bash_cmd(rx_connection, cmd, {})
        cmd = 'echo "DSOURCE_TYPE:{}" >> {}/.delphix/{}'.format(
            snapshot.d_source_type, mount_path, dataset_cfgfile
        )
        res = execute_bash_cmd(rx_connection, cmd, {})
        cmd = 'echo "MONGO_DB_USER:{}" >> {}/.delphix/{}'.format(
            snapshot.mongo_db_user, mount_path, dataset_cfgfile
        )
        res = execute_bash_cmd(rx_connection, cmd, {})

    if source_encrypted:
        cmd = 'echo "SOURCE_ENCRYPTED:{}" >> {}/.delphix/{}'.format(
            "True", mount_path, dataset_cfgfile
        )
        res = execute_bash_cmd(rx_connection, cmd, {})

        cmd = 'echo "ENCRYPTION_METHOD:{}" >> {}/.delphix/{}'.format(
            encryption_method, mount_path, dataset_cfgfile
        )
        res = execute_bash_cmd(rx_connection, cmd, {})
    else:
        cmd = 'echo "SOURCE_ENCRYPTED:{}" >> {}/.delphix/{}'.format(
            "False", mount_path, dataset_cfgfile
        )
        res = execute_bash_cmd(rx_connection, cmd, {})
    add_debug_space()
        ...
        if encryption_method == "KMIP":
            ...
            for kmip_params in sourceobj.parameters.kmip_params:
                enc_params_list_string = (
                    enc_params_list_string
                    + " --{} {}".format(
                        kmip_params["property_name"], kmip_params["value"]
                    )
                )
            ...
            if dsource_type == "shardedsource":
                kmip_key_id = get_kmip_key_id(
                    "{}/c{}m{}".format(mount_path, 0, 0), rx_connection
                )
                enc_params_list_string = (
                    enc_params_list_string
                    + " --kmipKeyIdentifier {}".format(kmip_key_id)
            ...
            logger.debug(
                "enc_params_list_string = {}".format(enc_params_list_string)
            )
        else:
            if dataset_type == "Staging":
                enc_params_list_string = (
                    " --enableEncryption --encryptionKeyFile {}".format(
            ...
            cmd = "cp -p {} {}/.delphix/.dlpx_enckeyfile".format(
                sourceobj.parameters.encryption_keyfile, mount_path
            )
            res = execute_bash_cmd(rx_connection, cmd, {})
    else:
    ...
    cmd = "{} --port {} --quiet --eval 'rs.conf()'".format(
        sourceobj.mongo_shell_path, start_portpool
    )
    res = execute_bash_cmd(rx_connection, cmd, {})

    cmd = "{} --port {} --quiet --eval 'rs.status()'".format(
        sourceobj.mongo_shell_path, start_portpool
    )
    res = execute_bash_cmd(rx_connection, cmd, {})
    ...
    if dsource_type == "shardedsource":
        if replicaset:
            add_debug_heading_block(
                "Replicaset - setup_config_replset_members"
            )
            setup_config_replset_members(
                shard_config_list,
                sourceobj,
                mount_path,
                encryption_method,
    ...
    if dsource_type == "shardedsource":
        ...
        cmd = "cat {}/c0m0/restoreInfo.txt|grep 'Replica Set:'".format(
            mount_path
        )
        res = execute_bash_cmd(rx_connection, cmd, {})
        config_repset_name = res.split(":")[1].strip()
        logger.debug("config_repset_name = {}".format(config_repset_name))
        configsvrstring = "{}/{}:{}".format(
            config_repset_name, hostname, start_portpool
        )
        ...
        if source_encrypted:
            for i in range(smax):
                ...
                if encryption_method == "KMIP":
                    kmip_key_id = get_kmip_key_id(
                        "{}/s{}m{}".format(mount_path, i, 0), rx_connection
                    )
                    enc_params_list_string = base_enc_params_list_string
                    enc_params_list_string = (
                        enc_params_list_string
                ...
                rs_initiate(
                    rx_connection, sourceobj.mongo_shell_path, snm0_port
                )
    ...
    if dsource_type == "shardedsource":
        if dataset_type == "Staging":
            ...
            create_mongoadmin_user(
                sourceobj, rx_connection, smax, shard_config_list
            )
            ...
            if (
                ...
                logger.info(
                    "Successfully formed sharded cluster using " "addShard()."
                )

        elif dataset_type == "Virtual":
            # Update mongo admin password
            update_mongoadmin_pwd(
                sourceobj,
                rx_connection,
                smax,
                shard_config_list,
                snapshot.mongo_db_user,
        ...
        if dataset_type == "Staging":
            if dsource_type != "extendedcluster":
                # Create mongo admin user
                create_mongoadmin_user(
                    sourceobj, rx_connection, 0, replicaset_config_list
                )
        elif dataset_type == "Virtual":
            # Update mongo admin password
            update_mongoadmin_pwd(
        ...
        add_debug_heading_block("Generate Config files")
        gen_config_files(
            dataset_type, sourceobj, replicaset_config_list, snapshot
        )
...
def setup_sharded_mongo_dataset(sourceobj, dataset_type, snapshot):
    ...
    if dataset_type == "Staging":
        ...
        for shardfile in sourceobj.parameters.shard_backupfiles:
            extract_shard_backup(
                shardfile["backupfile"], mount_path, shardnum, rx_connection
            )
            shardnum = shardnum + 1
        shardnum = shardnum - 1
...
def _validate_clustersync_user_inputs(source_obj) -> None:
    ...
    try:
        assert (
            str(source_obj.parameters.config_backupfile) == ""
...
def check_ops_center_backup_files(
    backup_file: str, os_lib_obj, raise_exception: bool = False
) -> int:
    """Checks if backup files created by Opscenter is available on host and is of format .tar / .tar.gz
    ...
    Raises:
        UserError: Backup file not available on given path
        UserError: Backup file not of format .tar / .tar.gz
    Returns:
        0: When file is present and valid.
        1: When file is not present.
        2: When file is not of type.tar /.tar.gz
    """
    configserver_backup_exists = os_lib_obj.check_file_dir_exists(
        path=backup_file,
        is_directory=False,
    )
    if not configserver_backup_exists:
        err_msg = "Backup file should be present on host. Given backup file not available. "
        if raise_exception:
            raise UserError(err_msg)
        return 1

    is_supported = os_lib_obj.is_file_tar_or_gz(tarfile_path=backup_file)
    if not is_supported:
        err_msg = "Backup file should be of type.tar /.tar.gz format. "
        if raise_exception:
            raise UserError(err_msg)
        return 2
    return 0
...
def check_mongodump_utility_dir(
    ...
    Args:
        ...
        raise_exception (bool)
    Raises:
        UserError: Raised when backup directory does not exist or a given path is not a directory
    Returns:
        0: All checks passed.
        1: Backup directory is not present.
    """
    mongodump_backup_dir_exists = os_lib_obj.check_file_dir_exists(
        path=backup_dir,
        is_directory=True,
    )
    if not mongodump_backup_dir_exists:
        err_msg = "Directory should be present on host. Given backup directory not available. "
        if raise_exception:
            raise UserError(err_msg)
        return 1
    return 0
...
def check_input_parameters(source_obj, is_dsource=False):
    """
    ...
    if is_dsource:
        ...
        if source_obj.parameters.d_source_type in dsource_types \
            ...
            if not source_obj.parameters.backup_metadata_file:
                raise UserError(
                    "Backup Metadata Filename must be provided, "
                    f"in case of DSource type: {source_obj.parameters.d_source_type}. "
                )
            # Check if config server file path is provided in input
            if not source_obj.parameters.config_backupfile:
                raise UserError(
                    "Config Server Backup File must be provided in input. "
                    f"in case of DSource type: {source_obj.parameters.d_source_type}. "
                )

        if source_obj.parameters.d_source_type == "offlinemongodump":
            # Check if input provided for mongdump backup directory is a directory
            return_code = check_mongodump_utility_dir(
                backup_dir=source_obj.parameters.config_backupfile,
                os_lib_obj=source_obj.os_lib_obj,
            )
            if return_code:
                err_msg = "Directory should be present on host. Given backup directory not available. "
                raise UserError(
                    f"Mongodump Backup directory: {source_obj.parameters.config_backupfile}, {err_msg} "
                )

        if (
            source_obj.parameters.d_source_type
            in ("shardedsource", "nonshardedsource")
        ) and not source_obj.parameters.enable_clustersync:
            return_code = check_ops_center_backup_files(
                backup_file=source_obj.parameters.config_backupfile,
                os_lib_obj=source_obj.os_lib_obj,
            )
            if return_code == 1:
                err_msg = "Backup file should be present on host. Given backup file not available. "
                raise UserError(
                    f"Backup file: {source_obj.parameters.config_backupfile}, {err_msg} "
                )
            if return_code == 2:
                err_msg = "Backup file should be of type.tar /.tar.gz format. "
                raise UserError(
        ...
        if source_obj.parameters.d_source_type == "shardedsource" \
            ...
            if not source_obj.parameters.shard_backupfiles:
                raise UserError(
                    ...
                    "in case of DSource type Sharded source. "
                )
            for shard_backup_dict in source_obj.parameters.shard_backupfiles:
                shard_backup = shard_backup_dict.get("backupfile")
                if shard_backup:
                    return_code = check_ops_center_backup_files(
                        backup_file=shard_backup,
                        os_lib_obj=source_obj.os_lib_obj,
                    )
                    if return_code == 1:
                        err_msg = "Backup file should be present on host. Given backup file not available. "
                        raise UserError(
                            f"Shard backup file: {source_obj.parameters.config_backupfile}, {err_msg} "
                        )
                    if return_code == 2:
                        err_msg = "Backup file should be of type.tar /.tar.gz format. "
                        raise UserError(
    ...
    if source_obj.parameters.enable_authentication:
        if (
            source_obj.parameters.cluster_auth_mode
            in ["keyFile", "sendKeyFile"]
            and not source_obj.parameters.keyfile_path
        ):
            raise UserError(
            ...
            if source_obj.parameters.enable_ssl_tls:
                tls_param_name_list = [
                    p["property_name"]
                    for p in source_obj.parameters.ssl_tls_params
                ]
                tls_expected_param = [
                    "tlsMode",
                    "tlsCAFile",
    ...
    if (
        hasattr(source_obj.parameters, "enable_clustersync")
        and source_obj.parameters.enable_clustersync
    ):
        _validate_clustersync_user_inputs(source_obj)

    if source_obj.parameters.ssl_tls_params and (
        source_obj.parameters.client_tls_cacert == ""
        or source_obj.parameters.client_tls_cert == ""
    ):
        raise UserError(
            "If SSL TLS parameters are specified, Please specify Client CA "
            "certificate and Client certificate for Mongos / Mongo session."
        )
...
def shutdown_server(rx_connection, mongo_shell_path: str, port: int) -> None:
    cmd = constants.Globals.SERVER_SHUTDOWN.format(
        mongo_shell_path=mongo_shell_path, port=port
    )
    try:
        execute_bash_cmd(rx_connection, cmd, {})
        time.sleep(5)
    except Exception as e:
        if not mongo_shell_path.split("/")[
            -1
        ] == "mongosh" and not _mongosh_marked_failed_shutdown(str(e)):
            err = constants.Globals.ERR_SERVER_SHUTDOWN.format(port=port)
            err += str(e)
            raise Exception(err)


def rs_initiate(rx_connection, mongo_shell_path: str, port: int) -> None:
    ...
    try:
        ...
        if not mongo_shell_path.split("/")[
            -1
        ] == "mongosh" and not _mongosh_marked_failed_rs_initiate(str(e)):
...
def create_new_file(os_lib_obj, file_path: str, content: str) -> None:
    try:
...
def _generate_restoreinfo_txt_for_config_server(
    ...
    create_new_file(
        os_lib_obj,
        file_path="{}/{}".format(
            mount_path, constants.Globals.CONFIG_SERVER_RESTORE_INFO_TXT_PATH
        ),
        content=content_str,
    )
...
def _generate_restoreinfo_txt_for_all_shards(
    ...
    for shard_data_dict in shard_details:
        ...
        content_str = "Replica Set: {}".format(shard_data_dict["_id"])
        create_new_file(
            os_lib_obj, file_path=restoreinfo_txt_path, content=content_str
        )
...
def _add_shard_id_in_shard_config_list(
    ...
    for shard_config_dict in shard_config_list:
        if shard_config_dict["type"] == "shard":
            ...
            restoreinfo_txt = constants.Globals.RESTOREINFO_TXT_NAME
            restoreinfo_txt_path = (
                f"{shard_config_dict['mount_path']}/{snm0}"
                f"/{restoreinfo_txt}"
            )
...
def read_db_user_for_vdb(sourceobj):
    stg_config_path = "{mount_path}/.delphix/.stg_config.txt".format(
        mount_path=sourceobj.parameters.mount_path
    )
    res = sourceobj.os_lib_obj.cat_file(file_path=stg_config_path)
    for line in res.splitlines():
        if "MONGO_DB_USER:" in line:
            return line.split(":")[-1]
    raise UserError("Failed to find Mongo database user.")


def check_and_remove_clustersync_internal_database(sourceobj):
    mongos_port = sourceobj.parameters.mongos_port

    cmd = "hostname"
    hostname = execute_bash_cmd(sourceobj.connection, cmd, {})

    mongo_db_user = read_db_user_for_vdb(sourceobj)
    mongo_db_password = sourceobj.parameters.mongo_db_password

    # NOTE:
    # Constructing the host_conn_string similar to the source database string
    # i.e. SSL/TLS parameters appear as command line options itself.
    # For example,
    # mk-db.dcol1.delphix.com:28501 --tls --tlsCertificateKeyFile s0m0.pem
    #   --tlsCAFile mongoCA.crt
    # Accepting this, MongoDB::get_standard_conn_string() would parse the
    # construct the MongoDB qualified connection string later on.
    host_conn_string = f"{hostname}:{mongos_port}"
    if (
        sourceobj.parameters.client_tls_cert
        and sourceobj.parameters.client_tls_cacert
    ):
        host_conn_string = (
            "{} --tls --tlsCAFile {}"
            " --tlsCertificateKeyFile {}".format(
                host_conn_string,
                sourceobj.parameters.client_tls_cacert,
                sourceobj.parameters.client_tls_cert,
            )
        )

    logger.debug(
        "Checking if {} database exists.".format(
            MongoDBLibConstants.CLUSTERSYNC_RESERVED_DATABASE
        )
    )
    if sourceobj.mongodb_obj.db_exists(
        ...
        logger.debug(
            "Removing {} database as it is no more required.".format(
                MongoDBLibConstants.CLUSTERSYNC_RESERVED_DATABASE
            )
        )
        sourceobj.mongodb_obj.drop_database(
            host_conn_string=host_conn_string,
            username=mongo_db_user,
            password=mongo_db_password,
            db_name=MongoDBLibConstants.CLUSTERSYNC_RESERVED_DATABASE,
