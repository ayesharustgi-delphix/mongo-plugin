...
def _handle_exit_code(exit_code, std_err=None, std_output=None,
    ...
    logger.debug(
        "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@  ERRROR  @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
    logger.debug(
        "cmd Exit Code: {}, Error Msg : {}".format(exit_code, error_details))
    add_debug_space()
...
def _get_mongod_logpath_from_commandline(mongod_cmd: str) -> str:
    ...
    try:
        return mongod_cmd.split("--logpath")[1].split()[0]
    except IndexError:
        errmsg = "No logpath found in the mongod command line[{}].".format(
            mongod_cmd
        )
        raise Exception(errmsg)


def _get_mongod_logpath_from_conf(connection, mongod_cmd, env) -> str:
    ...
    try:
        mongod_conf_file = mongod_cmd.split('-f')[-1].split()[0]
        logger.debug("mongod conf file:{}".format(mongod_conf_file))
    except IndexError:
        errmsg = "No conf file found in the mongod command line[{}].".format(
            mongod_cmd
        )
        raise Exception(errmsg)

    cmd = "grep 'path:' {}".format(mongod_conf_file)
    res = libs.run_bash(connection, cmd, env)
    exit_code = res.exit_code
    try:
        assert exit_code == 0, \
            "Failed to execute {}. 'path' is not found in {}.".format(
                cmd,
                mongod_conf_file
            )
...
def _get_mongod_logpath(connection, mongod_cmd, env) -> str:
    if '--logpath' in mongod_cmd:
        return _get_mongod_logpath_from_commandline(mongod_cmd)
    elif ' -f ' in mongod_cmd:
        return _get_mongod_logpath_from_conf(connection, mongod_cmd, env)


def _get_mongod_error_from_logpath(connection, logpath, env) -> str:
    cmd = "grep 'error\":' {}".format(logpath)
    res = libs.run_bash(connection, cmd, env)
    exit_code = res.exit_code
    try:
        assert exit_code == 0, "No errors are found in the {}".format(
            logpath
        )
    except AssertionError as e:
        raise Exception(str(e))
    ...
    mongo_errmsg_raw = json.loads(mongo_full_errmsg_raw)['attr']['error']
    return mongo_errmsg_raw


def get_mongod_error(connection, mongod_cmd, env) -> str:
    ...
    logpath = _get_mongod_logpath(connection, mongod_cmd, env)
    logger.debug("logpath {}".format(logpath))

    # Check if any 'error' messages are found in the log file.
    return _get_mongod_error_from_logpath(
        connection,
        logpath,
        env
    )
...
def execute_bash_cmd(connection, cmd, env):
    ...
    if exit_code != 0 and 'the "--fork" option' in res.stdout:
        ...
        try:
            mongod_error_raw = get_mongod_error(connection, cmd, env)
            # Append the error message that we got from the logpath to the
            # original errmsg
            errmsg = "{}\n{}".format(errmsg, mongod_error_raw)
        except Exception as e:
...
def setup_config_member(sourceobj, rx_connection, mount_path, confignum,
    ...
    cmd = "{} admin --port {} --quiet --eval 'db.shutdownServer()'".format(
        sourceobj.mongo_shell_path, cfg_port)
    res = execute_bash_cmd(rx_connection, cmd, {})
    time.sleep(7)
...
def setup_config_replset_members(shard_config_list, sourceobj, mount_path,
    ...
    for i in range(1, 3):
        ...
        cmd = "{} admin --port {} --quiet --eval 'db.shutdownServer()'".format(
            sourceobj.mongo_shell_path, cnmn_port)
        res = execute_bash_cmd(cnmn_conn, cmd, {})
        time.sleep(7)
...
def setup_dataset(sourceobj, dataset_type, snapshot, dsource_type):
    ...
    cmd = "{} --port {} --quiet --eval 'rs.initiate()'".format(
