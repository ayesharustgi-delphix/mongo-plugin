...
def gen_shard_config_list(nodes, start_portpool, cmax, smax, mmax, mount_path,
                          mongos_port, replicaset):
    if replicaset:
        mmax = 3
    else:
        mmax = 1
    ...
    if totalnodes == 1:
        for i in range(cmax):
            for j in range(mmax):
                shard_cfg_dict = {}
                shard_cfg_dict = {"node": nodes[n], "type": "config",
                                  "mount_path": "{}".format(
                                      mount_path),
    ...
    if totalnodes == 1:
        for i in range(smax):
            for j in range(mmax):
                shard_cfg_dict = {}
                shard_cfg_dict = {"node": nodes[n], "type": "shard",
                                  "mount_path": "{}".format(
                                      mount_path),
        ...
        for i in range(smax):
            ...
            logger.debug(" ")
    n = 0
    shard_cfg_dict = {"node": nodes[n], "type": "mongos",
                      "mount_path": "{}".format(
                          mount_path), "dirname": "mgs{}".format(0),
                      "port": mongos_port}
    logger.debug(shard_cfg_dict)
    logger.debug(" ")
    shard_cfg_list.append(shard_cfg_dict)
...
def gen_replicaset_config_list(nodes, start_portpool, mount_path, replicaset):
    ...
    if totalnodes == 1:
        for i in range(1):
            for j in range(mmax):
                replicaset_cfg_dict = {}
                replicaset_cfg_dict = {"node": nodes[n], "type": "repset",
                                       "mount_path": "{}".format(
                                           mount_path),
                                       "dirname": "s{}m{}".format(i, j),
                                       "port": mongoport}
...
def _handle_exit_code(exit_code, std_err=None, std_output=None,
                      callback_func=None, silent_status='N'):
    if exit_code == 0:
        if silent_status == "N":
            logger.debug(
    ...
    logger.debug(
        "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@  ERRROR  @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
    logger.debug(
        "cmd Exit Code: {}, Error Msg : {}, Output Msg : {}".format(exit_code,
                                                                    std_err,
                                                                    std_output))
    add_debug_space()
...
def gen_shardserver_setting_list(shard_config_list, sourceobj, shard_count,
                                 mount_path, dataset_type='Virtual'):
    shard_setting_list = []
...
def gen_mongo_conf_files(dataset_type, sourceobj, shard_config_list, snapshot):
    ...
    for rec in json_result.split("},"):
        ...
        cfgfile = "{}/dlpx.{}.{}.conf".format(cfgdir, mongod_dirname,
                                              mongod_port)
        logfile = "{}/dlpx.{}.{}.mongod.log".format(logdir, mongod_dirname,
                                                    mongod_port)
        ...
        mongo_cmd = add_net(mongo_cmd, bind_ip, mongod_port, enable_ssl_tls,
                            ssl_tls_params)
        ...
        mongo_cmd = add_set_parameters(mongo_cmd, enable_setparams,
                                       setparam_params)
        logger.info(
        ...
        if mongo_process_type == "mongos":
            ...
            if source_encrypted:
                ...
                if encryption_method == "KMIP":
                    ...
                    if d_source_type not in ["offlinemongodump",
                                             "onlinemongodump",
                                             "extendedcluster"]:
                        ...
                        mongo_cmd = mongo_cmd + " --kmipKeyIdentifier {}".format(
                            kmip_key_id)
                        logger.info(
                    ...
                    mongo_cmd = add_encryption_keyfile(mongo_cmd,
                                                       encryption_keyfile)
                    logger.info(
        ...
        if mongo_process_type == "shard":
            mongo_cmd = "{} --replSet {} --shardsvr".format(mongo_cmd,
                                                            repset_name)
        elif mongo_process_type == "config":
            mongo_cmd = "{} --replSet {} --configsvr".format(mongo_cmd,
                                                             repset_name)
        ...
        if mongo_process_type == "mongos":
            ...
            cfgdblist = gen_configsvrstring(dataset_type, sourceobj,
                                            shard_config_list)
...
def stop_sharded_mongo(dataset_type, sourceobj):
    ...
    if d_source_type == "stagingpush" and dataset_type == "Staging":
        ...
        for rec in json_result.split("},"):
            ...
            logger.debug("Record = {}".format(rec))
            mongod_port = jsonobj['port']
            mongod_conn = get_node_conn(sourceobj, jsonobj['node'],
                                        dataset_type)
            mongod_dirname = jsonobj['dirname']
    ...
    try:
        ...
        logger.debug(
            "ERROR : Checking status of sharded mongo - Mongos {} at {}".format(
                sourceobj.parameters.mongos_port,
                mount_path))
        logger.debug(e)
        active_status_flag = 1
        return active_status_flag
...
def create_mongoadmin_user(sourceobj, connection, shard_count,
                           shard_config_list, resync=False, dsource_type=None):
...
def update_mongoadmin_pwd(sourceobj, connection, shard_count, shard_config_list,
...
def fsync_lock_sharded_mongo(sourceobj, dataset_type='Virtual'):
    ...
    # Stop Balancer
    cmd = "{} --port {} --eval 'sh.stopBalancer()'".format(mongo_shell_cmd,
                                                           mongos_port)
    logger.info("cmd = {}".format(cmd))
    res = execute_bash_cmd(connection, cmd, {})
    ...
    for rec in json_result.split("},"):
        ...
        if jsonobj['type'] != 'mongos':
            cmd = "{} --port {} --eval 'db.fsyncLock()'".format(mongo_shell_cmd,
                                                                mongod_port)
...
def fsync_unlock_sharded_mongo(sourceobj, dataset_type='Virtual'):
    ...
    # Start Balancer
    mongos_port = sourceobj.parameters.mongos_port
    cmd = "{} --port {} --eval 'sh.startBalancer()'".format(mongo_shell_cmd,
                                                            mongos_port)
    logger.info("cmd = {}".format(cmd))
    res = execute_bash_cmd(connection, cmd, {})
...
def setup_mongos(sourceobj, rx_connection, mount_path, membernum, mongos_port,
                 configsvrstring, shard_config_list):
    ...
    logfile = "{}/dlpx.mgs{}.{}.mongod.log".format(logdir, membernum,
                                                   mongos_port)
...
def setup_config_member(sourceobj, rx_connection, mount_path, confignum,
                        membernum, cfg_port, shardcount,
                        shard_config_list,
    ...
    cfgdir = "{}/cfg".format(mount_path)
    logdir = "{}/logs".format(mount_path)
    cfgfile = "{}/dlpx.c{}m{}.{}.conf".format(cfgdir, confignum, membernum,
                                              cfg_port)
    logfile = "{}/dlpx.c{}m{}.{}.mongod.log".format(logdir, confignum,
                                                    membernum, cfg_port)

    cmd = "hostname"
    hostname = execute_bash_cmd(rx_connection, cmd, {})
    logger.debug("{},{},{},{},{}".format(dbpath, cfg_port, shardcount, cfgfile,
                                         hostname))

    cmd = "cat {}/c{}m{}/restoreInfo.txt|grep 'Replica Set:'".format(mount_path,
                                                                     confignum,
                                                                     membernum)
    res = execute_bash_cmd(rx_connection, cmd, {})
    ...
    if encryption_method is None:
        ...
        restart_mongod_cmd = "{} --bind_ip 0.0.0.0 --port {} --dbpath {} --logpath {} {} --fork --replSet {} --configsvr".format(
            sourceobj.mongo_install_path,
            cfg_port, dbpath, logfile, enc_params_list_string,
            config_repset_name)
    ...
    if dataset_type == "Virtual" \
        ...
        for i in range(shardcount):
            ...
            adjust_config_shardinfo(sourceobj, rx_connection, shard_dbpath,
                                    cfg_port, hostname, snm0_port,
                                    shardserver_setting_list)
...
def setup_config_replset_members(shard_config_list, sourceobj, mount_path,
                                 encryption_method,
    ...
    for i in range(1, 3):
        ...
        cnmn_port = get_shard_port(shard_config_list,
                                   'c{}m{}'.format(confignum, membernum))
        cnmn_host = get_shard_host(shard_config_list,
                                   'c{}m{}'.format(confignum, membernum))
        ...
        cmd = "cp -p {}/c0m0/restoreInfo.txt {}/c{}m{}/restoreInfo.txt".format(
            mount_path, mount_path, confignum,
            membernum)
        ...
        if encryption_method is None:
            ...
            restart_mongod_cmd = "{} --bind_ip 0.0.0.0 --port {} --dbpath {} --logpath {} {} --fork --replSet {} --configsvr".format(
                sourceobj.mongo_install_path,
                cnmn_port, dbpath, logfile, enc_params_list_string,
                config_repset_name)
        ...
        logger.info(
            "Add replicaset Member c{}m{} to replicaset {}".format(confignum,
                                                                   membernum,
                                                                   config_repset_name))
        cmd = '{} --port {} --eval \'rs.add("{}:{}")\''.format(
            sourceobj.mongo_shell_path, c0m0_port, cnmn_host_name,
            cnmn_port)
...
def setup_shard_replset_members(shard_config_list, virtual_source, mount_path,
                                encryption_method,
    ...
    for shardnum in range(shard_count):
        ...
        if encryption_method == "KMIP":
            ...
            enc_params_list_string = enc_params_list_string + " --kmipKeyIdentifier {}".format(
                kmip_key_id)
        else:
            kmip_key_id = None
        ...
        for i in range(1, 3):
            ...
            snmn_port = get_shard_port(shard_config_list,
                                       's{}m{}'.format(shardnum, membernum))
            snmn_host = get_shard_host(shard_config_list,
                                       's{}m{}'.format(shardnum, membernum))
            ...
            cfgdir = "{}/cfg".format(mount_path)
            logdir = "{}/logs".format(mount_path)
            cfgfile = "{}/dlpx.s{}m{}.{}.conf".format(cfgdir, shardnum,
                                                      membernum, snmn_port)
            logfile = "{}/dlpx.s{}m{}.{}.mongod.log".format(logdir, shardnum,
                                                            membernum,
                                                            snmn_port)
            ...
            if encryption_method is None:
                ...
                restart_mongod_cmd = "{} --bind_ip 0.0.0.0 --port {} --dbpath {} --logpath {} {} --fork --replSet {} --shardsvr".format(
                    virtual_source.mongo_install_path,
                    snmn_port, dbpath, logfile, enc_params_list_string,
                    shard_repset_name)
            ...
            shutdown_server(snmn_conn, virtual_source.mongo_shell_path,
                            snmn_port)
            ...
            # Add Member to replicaset
            logger.info(
                "Add replicaset Member s{}m{} to replicaset {}".format(shardnum,
                                                                       membernum,
                                                                       shard_repset_name))
            cmd = '{} --port {} --eval \'rs.add("{}:{}")\''.format(
                virtual_source.mongo_shell_path, snm0_port,
...
def add_replset_members(sourceobj, dataset_type, start_portpool,
                        replicaset_config_list, mount_path, encryption_method,
                        enc_params_list_string):
    if dataset_type == "Virtual":
        cfgfile = "{}/.delphix/.tgt_vdbcfg.txt".format(mount_path)
        rx_connection = sourceobj.connection
    ...
    for i in range(1, 3):
        ...
        snmn_port = get_shard_port(replicaset_config_list,
                                   's0m{}'.format(membernum))
        snmn_host = get_shard_host(replicaset_config_list,
                                   's0m{}'.format(membernum))
        ...
        logfile = "{}/dlpx.s0m{}.{}.mongod.log".format(logdir, membernum,
                                                       snmn_port)
        ...
        # Add Member to replicaset
        logger.info(
            "Add replicaset Member s0m{} to replicaset {}".format(membernum,
                                                                  repset_name))
        cmd = '{} --port {} --eval \'rs.add("{}:{}")\''.format(
            sourceobj.mongo_shell_path, s0m0_port, snmn_host_name,
            snmn_port)
...
def adjust_config_shardinfo(sourceobj, rx_connection, shard_dbpath, cfg_port,
                            hostname, shard_port,
                            shardserver_setting_list):
...
def setup_shard_member(sourceobj, rx_connection, mount_path, shardnum,
                       membernum, snm0_port, shardcount,
                       shard_config_list,
                       encryption_method, enc_params_list_string,
                       configsvrstring):
    dbpath = "{}/s{}m{}".format(mount_path, shardnum, membernum)
    cfgdir = "{}/cfg".format(mount_path)
    logdir = "{}/logs".format(mount_path)
    cfgfile = "{}/dlpx.s{}m{}.{}.conf".format(cfgdir, shardnum, membernum,
                                              snm0_port)
    logfile = "{}/dlpx.s{}m{}.{}.mongod.log".format(logdir, shardnum, membernum,
                                                    snm0_port)

    cmd = "hostname"
    hostname = execute_bash_cmd(rx_connection, cmd, {})
    logger.debug("{},{},{},{},{}".format(dbpath, snm0_port, shardcount, cfgfile,
                                         hostname))

    cmd = "cat {}/s{}m{}/restoreInfo.txt|grep 'Replica Set:'".format(mount_path,
                                                                     shardnum,
                                                                     membernum)
    res = execute_bash_cmd(rx_connection, cmd, {})
    shard_repset_name = res.split(':')[1].strip()
    ...
    if encryption_method is None:
        ...
        restart_mongod_cmd = "{} --bind_ip 0.0.0.0 --port {} --dbpath {} --logpath {} {} --fork --replSet {} --shardsvr".format(
            sourceobj.mongo_install_path,
            snm0_port, dbpath, logfile, enc_params_list_string,
            shard_repset_name)
...
def setup_replicaset_member(sourceobj, rx_connection, mount_path, cfg_port,
                            encryption_method, enc_params_list_string,
                            dsource_type):
    ...
    if dsource_type == "extendedcluster":
        ...
        cmd = "{} local --port {} --quiet --eval 'db.getSiblingDB(\"local\").system.replset.insert({{\"_id\" : \"{}\", \"version\" : NumberInt(1), \"protocolVersion\" : NumberInt(1), \"members\" : [{{ \"_id\" : NumberInt(0), \"host\" : \"{}:{}\"}}], \"settings\" : {{ }} }})'".format(
            sourceobj.mongo_shell_path, cfg_port, repset_name, hostname,
            cfg_port)
        res = execute_bash_cmd(rx_connection, cmd, {})
...
def add_cluster_auth(mongo_cmd, cluster_auth_mode, keyfile_path):
    ...
    if cluster_auth_mode == "None":
        ...
        mongo_cmd = "{} --clusterAuthMode {}".format(mongo_cmd,
                                                     cluster_auth_mode)
    return mongo_cmd
    ...
    if enable_setparams:
        ...
        for set_params_rec in setparam_params:
            if set_params_rec['value'].upper() == "TRUE":
                set_params_list_string = set_params_list_string + " --{}".format(
    ...
    if kmip_params:
        ...
        for kmip_params_rec in kmip_params:
            if kmip_params_rec['value'].upper() == "TRUE":
                kmip_params_list_string = kmip_params_list_string + " --{}".format(
...
def add_encryption_keyfile(mongo_cmd, enc_key_file_name):
    if enc_key_file_name:
        mongo_cmd = "{} --enableEncryption".format(mongo_cmd)
        mongo_cmd = "{} --encryptionKeyFile {}".format(mongo_cmd,
                                                       enc_key_file_name)
    return mongo_cmd
...
def gen_config_files(dataset_type, sourceobj, shard_config_list, snapshot=None):
    ...
    if dataset_type == "Virtual":
        gen_mongo_conf_files(dataset_type, sourceobj, shard_config_list,
                             snapshot)
    else:
...
def setup_dataset(sourceobj, dataset_type, snapshot, dsource_type):
    ...
    if dsource_type == "shardedsource":
        ...
        shard_config_list = gen_shard_config_list(
            nodes, start_portpool, cmax, smax, mmax, mount_path, mongos_port,
            replicaset)
        for shard_config in shard_config_list:
    ...
    if dataset_type == 'Staging':
        if dsource_type == "shardedsource":
            if sourceobj.parameters.enable_clustersync:
                ...
                # restore backups
                extract_shard_config_backup(config_backupfile, mount_path, 0,
                                            rx_connection)
                shardnum = 0
                for shardfile in sourceobj.parameters.shard_backupfiles:
                    extract_shard_backup(shardfile['backupfile'], mount_path,
                                         shardnum, rx_connection)
                    shardnum = shardnum + 1
                shardnum = shardnum - 1
                logger.debug(
            ...
            # restore backups
            extract_replicaset_backup(config_backupfile, mount_path,
                                      rx_connection)
    ...
    if dataset_type == 'Staging':
        if dsource_type == "shardedsource":
            cmd = "echo \"SHARD_COUNT:{}\" > {}/.delphix/{}".format(smax,
                                                                    mount_path,
                                                                    dataset_cfgfile)
            res = execute_bash_cmd(rx_connection, cmd, {})
        elif dsource_type == "nonshardedsource":
            cmd = "cat /dev/null > {}/.delphix/{}".format(mount_path,
                                                          dataset_cfgfile)
            res = execute_bash_cmd(rx_connection, cmd, {})

        cmd = "echo \"DSOURCE_TYPE:{}\" >> {}/.delphix/{}".format(
            sourceobj.parameters.d_source_type, mount_path,
            dataset_cfgfile)
        res = execute_bash_cmd(rx_connection, cmd, {})
        ...
        if dsource_type == "shardedsource":
            ...
            res = execute_bash_cmd(rx_connection, cmd, {})
                              ...
                              "onlinemongodump", "extendedcluster",
                              "stagingpush", "seed"]:
            cmd = "cat /dev/null > {}/.delphix/{}".format(mount_path,
                                                          dataset_cfgfile)
            res = execute_bash_cmd(rx_connection, cmd, {})
        ...
        res = execute_bash_cmd(rx_connection, cmd, {})
        cmd = "echo \"MONGO_DB_USER:{}\" >> {}/.delphix/{}".format(
            snapshot.mongo_db_user, mount_path,
            dataset_cfgfile)
        res = execute_bash_cmd(rx_connection, cmd, {})

    if source_encrypted:
        cmd = "echo \"SOURCE_ENCRYPTED:{}\" >> {}/.delphix/{}".format("True",
                                                                      mount_path,
                                                                      dataset_cfgfile)
        res = execute_bash_cmd(rx_connection, cmd, {})
        ...
        cmd = "echo \"SOURCE_ENCRYPTED:{}\" >> {}/.delphix/{}".format("False",
                                                                      mount_path,
                                                                      dataset_cfgfile)
        res = execute_bash_cmd(rx_connection, cmd, {})
    add_debug_space()
    ...
    if source_encrypted:
        if encryption_method == "KMIP":
            ...
            for kmip_params in sourceobj.parameters.kmip_params:
                enc_params_list_string = enc_params_list_string + " --{} {}".format(
                    kmip_params['property_name'],
                    kmip_params['value'])
    ...
    if dsource_type == "shardedsource":
        ...
        if source_encrypted:
            ...
            res = setup_config_member(sourceobj, rx_connection, mount_path,
                                      confignum, membernum, start_portpool,
                                      smax,
                                      shard_config_list, encryption_method,
                                      enc_params_list_string,
                                      shardserver_setting_list,
        ...
        if source_encrypted:
            ...
            res = setup_replicaset_member(sourceobj, rx_connection, mount_path,
                                          start_portpool, encryption_method,
                                          enc_params_list_string, dsource_type)

        else:
            add_debug_heading_block("Unencrypted - setup_replicaset_member")
            res = setup_replicaset_member(sourceobj, rx_connection, mount_path,
                                          start_portpool, None, None,
                                          dsource_type)
    ...
    if dsource_type == "shardedsource":
        ...
        if replicaset:
            ...
            setup_config_replset_members(shard_config_list, sourceobj,
                                         mount_path, encryption_method,
                                         enc_params_list_string, dataset_type)
        ...
        if replicaset:
            ...
            logger.info(
                "{},{},{},{},{},{} ".format(dataset_type, start_portpool,
                                            replicaset_config_list, mount_path,
                                            encryption_method,
                                            enc_params_list_string))
            add_replset_members(sourceobj, dataset_type, start_portpool,
                                replicaset_config_list, mount_path,
                                encryption_method, enc_params_list_string)
            add_debug_space()
    ...
    if dsource_type == "shardedsource":
        ...
        config_repset_name = res.split(':')[1].strip()
        logger.debug("config_repset_name = {}".format(config_repset_name))
        configsvrstring = "{}/{}:{}".format(config_repset_name, hostname,
                                            start_portpool)
        ...
        if source_encrypted:
            for i in range(smax):
                ...
                setup_shard_member(sourceobj, rx_connection, mount_path,
                                   shardnum, membernum, snm0_port, smax,
                                   shard_config_list, encryption_method,
                                   enc_params_list_string, configsvrstring)

                rs_initiate(rx_connection, sourceobj.mongo_shell_path,
                            snm0_port)
        ...
        if replicaset:
            ...
            setup_shard_replset_members(shard_config_list, sourceobj,
                                        mount_path, encryption_method,
        ...
        setup_mongos(sourceobj, rx_connection, mount_path, membernum,
                     mongos_port, configsvrstring, shard_config_list)

    if dsource_type == "shardedsource":
        if dataset_type == 'Staging':
            # Create mongo admin user
            create_mongoadmin_user(sourceobj, rx_connection, smax,
                                   shard_config_list)
            ...
            update_mongoadmin_pwd(sourceobj, rx_connection, smax,
                                  shard_config_list, snapshot.mongo_db_user,
        ...
        if dataset_type == 'Staging':
            if dsource_type != "extendedcluster":
                # Create mongo admin user
                create_mongoadmin_user(sourceobj, rx_connection, 0,
                                       replicaset_config_list)
        elif dataset_type == 'Virtual':
            # Update mongo admin password
            update_mongoadmin_pwd(sourceobj, rx_connection, 0,
                                  replicaset_config_list,
        ...
        # Generate Config files
        add_debug_heading_block("Generate Config files")
        gen_config_files(dataset_type, sourceobj, replicaset_config_list,
                         snapshot)
...
def setup_sharded_mongo_dataset(sourceobj, dataset_type, snapshot):
    ...
    if dataset_type == 'Staging':
        ...
        shardnum = 0
        for shardfile in sourceobj.parameters.shard_backupfiles:
            extract_shard_backup(shardfile['backupfile'], mount_path, shardnum,
                                 rx_connection)
            shardnum = shardnum + 1
        shardnum = shardnum - 1
    ...
    cmd = "echo \"SHARD_COUNT:{}\" > {}/.delphix/{}".format(smax, mount_path,
                                                            dataset_cfgfile)
    res = execute_bash_cmd(rx_connection, cmd, {})
    ...
    if dataset_type == 'Staging':
        ...
        cmd = "echo \"MONGO_DB_USER:{}\" >> {}/.delphix/{}".format(
            sourceobj.parameters.mongo_db_user, mount_path,
            dataset_cfgfile)
        res = execute_bash_cmd(rx_connection, cmd, {})
    ...
    if source_encrypted:
        if encryption_method == "KMIP":
            ...
            kmip_key_id = get_kmip_key_id("{}/c{}m{}".format(mount_path, 0, 0),
                                          rx_connection)
            enc_params_list_string = enc_params_list_string + " --kmipKeyIdentifier {}".format(
                kmip_key_id)
    ...
    if source_encrypted:
        add_debug_heading_block("Encrypted - setup_config_member")
        res = setup_config_member(rx_connection, mount_path, confignum,
                                  membernum, start_portpool, smax,
                                  shard_config_list, encryption_method,
                                  enc_params_list_string,
                                  shardserver_setting_list,
                                  dataset_type=dataset_type)
    ...
    if replicaset:
        add_debug_heading_block("Replicaset - setup_config_replset_members")
        setup_config_replset_members(shard_config_list, sourceobj, mount_path,
                                     encryption_method,
                                     enc_params_list_string, dataset_type)
    ...
    if source_encrypted:
        for i in range(smax):
            ...
            setup_shard_member(sourceobj, rx_connection, mount_path, shardnum,
                               membernum, snm0_port, smax,
                               shard_config_list, encryption_method,
                               enc_params_list_string, configsvrstring)
    ...
    if replicaset:
        add_debug_heading_block("Replicaset - setup_shard_replset_members")
        logger.debug("setup_shard_replset_members() - Start")
        setup_shard_replset_members(shard_config_list, sourceobj, mount_path,
                                    encryption_method,
                                    base_enc_params_list_string, smax,
                                    dataset_type)
        logger.debug("setup_shard_replset_members() - End")
    ...
    # setup mongos
    add_debug_heading_block("setup mongos")
    setup_mongos(sourceobj, rx_connection, mount_path, membernum, mongos_port,
                 configsvrstring, shard_config_list)
    ...
    if dataset_type == 'Staging':
        ...
        # Update mongo admin password
        update_mongoadmin_pwd(sourceobj, rx_connection, smax, shard_config_list,
                              snapshot.mongo_db_user,
...
def check_input_parameters(source_obj):
    if source_obj.parameters.enable_authentication:
        if (source_obj.parameters.cluster_auth_mode in ["keyFile",
                                                        "sendKeyFile"] and
    ...
    if source_obj.parameters.ssl_tls_params \
            and (
            source_obj.parameters.client_tls_cacert == '' \
            or
            source_obj.parameters.client_tls_cert == ''
    ):
        raise UserError(
...
def check_and_remove_clustersync_internal_database(sourceobj):
    ...
    if sourceobj.parameters.client_tls_cert \
            and sourceobj.parameters.client_tls_cacert:
        host_conn_string = "{} --tls --tlsCAFile {}" \
    ...
    logger.debug("Checking if {} database exists.".format(
    ...
    if sourceobj.mongodb_obj.db_exists(
        ...
        logger.debug("Removing {} database as it is no more required.".format(
            MongoDBLibConstants.CLUSTERSYNC_RESERVED_DATABASE
        ))
